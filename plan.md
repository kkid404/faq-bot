...
* [ ] В `/core`:
  ...
  * [ ] Добавить `main.py` с `/health`
  * [ ] Настроить `uvicorn` для локального запуска

* [ ] В корне проекта:

  * [ ] Написать `docker-compose.yml` с:

    * `core`
    * `db` (Postgres)
  * [ ] Убедиться, что `core` может подключиться к БД (хотя бы просто коннект).
...
* [ ] Описать Pydantic-схемы:
  ...
  * [ ] `DemoMessageRequest { session_id: str, text: str }`
  * [ ] `DemoMessageResponse { reply_text: str }`

* [ ] Добавить файл с логикой демо, типа `demo_logic.py`:

  * [ ] Завести "интенты" (цена, запись, адрес, график, услуги) с наборами ключевых слов и фраз.
  * [ ] По входящему тексту находить подходящий интент по простому скорингу (кол-во совпавших ключевых слов).
  * [ ] Для каждого интента возвращать шаблон ответа, подставляя данные демо-бизнеса.
  * [ ] Если интент не найден, вежливо просить уточнить вопрос или предлагать основные варианты ("узнать цену", "записаться", "уточнить адрес").

* [ ] Добавить эндпоинт:

  * [ ] `POST /demo/message` → принимает `DemoMessageRequest`, возвращает `DemoMessageResponse`

* [ ] Проверить через `/docs`:

  * [ ] Отправить несколько тестовых сообщений
  * [ ] Посмотреть, как меняются ответы
...
* детализировать **конкретно Блок 2 + 3** (то, с чего ты начнёшь прямо сейчас),
* или я могу помочь написать **конкретный TODO.md** для репозитория на основе этого плана (готовый текст файла, который ты просто закинешь в корень проекта).
