...
* [X] Инициализировать фронт-проект Next.js.

* [X] Сделать простую страницу `index` с базовой структурой:
  ...
  * [X] **Блок “Как это работает”** (3 шага)
  ...
  * [X] **Блок с демо-чатом**:
    ...
    * Компонент `<DemoChat />`, который:
      ...
      * [X] хранит массив сообщений `{ from: 'user' | 'bot', text }`
      * [X] рендерит пузырьки
      * [X] по Enter/кнопке отправляет запрос `POST /demo/message`
      * [X] показывает “бот печатает…” при ожидании ответа
...
* [X] Настроить CORS на бекенде, чтобы фронт мог ходить к API.
...
* [ ] Вариант продукта для компаний: бот по внутренней документации (knowledge-bot), который ищет ответы в корпоративных документах на том же core/ai-service.
